...
export const intro = {
    ...
    intro: `
    주어진 기능을 구현하는 데 그치지 않습니다. 운영 중인 시스템이 왜 느리고 왜 문제가 생기는지 스스로 질문을 던지고, 원인을 근본까지 추적해 다시 설계합니다.

    조회가 느린 이유를 데이터베이스 인덱스 구조까지 파고들어 개선했고, 긴급 제어 명령이 하나도 전송되지 않던 치명 오류를 좌표 처리 단계까지 거슬러 올라가 해결했습니다. 문제를 덮지 않고 끝까지 책임지는 것이 제 방식입니다.

    NestJS·Python으로 REST API와 실시간 통신 서버를 설계하며, 서버에 머무르지 않고 그 서버가 실제로 도는 현장 장비와 운영 환경까지 직접 챙깁니다. 필요한 일이면 경계를 긋지 않고 먼저 나서서 완성합니다.
    ...
    contents: [
        {
            ...
            desc: [
                ...
                { main: "고속철도 열차제어설비(TFM) 모니터링·시뮬레이션 통합 개발", sub: [
                    ...
                    "연동장치를 대신하는 시뮬레이터를 공유 메모리 기반으로 구현 (구간별 50개+ 설비 실시간 처리)",
        ...
        {
            title: "글로벌메타",
            subtitle: "연구개발 연구원 · 교통관제·ITS 백엔드 / 철도 계측기 (연구소 SAIG 이전 · 프로젝트 연속)",
            period: "2025.01 ~ 2025.12",
            link: "",
            desc: [
                { main: "교통관제 백엔드 조회 성능 개선", sub: [
                    "시간 컬럼을 문자열 LIKE로 검색해 인덱스를 못 타던 쿼리를 범위 조건(sargable)으로 재작성",
                ...
                { main: "긴급차 우선신호 시스템 개편 (Node.js → Python)", sub: [
                    "구형 Node.js를 Python(asyncio) 비동기 서버로 전면 개편, 단일 → 최대 5대 동시 운행 확장",
                    "위경도→평면좌표 변환·진입 방향 4방위 분류·45m 반경 기반 신호 제어 경로 탐색 로직 직접 설계",
                    "긴급 제어 명령이 0건 전송되던 치명 오류를 좌표 처리 단계까지 추적해 해결, 실주행 경로(72지점·6교차로)로 검증",
                    "인터넷 없는 CentOS 7에 Python 소스 빌드로 오프라인 배포, PM2 자동 재기동",
                ] },
                { main: "좌회전 감응 신호 시스템 (영상 AI)", sub: [
                    "엣지 장비(Jetson Nano)에 YOLO(v5→v11)로 좌회전 대기 차량 실시간 인식, 안성·장흥 실도로 배포",
                    "신호제어기 단방향 시리얼 통신을 웹 실시간 구독 양방향 구조로 확장 (통신 방식 추상화)",
                ] },
                { main: "철도 신호용 계전기 시험기 개발", sub: [
                    "산업용 보드(Allwinner T507)에 Ubuntu·Python 직접 빌드, 계전기 6종 저항 자동 측정·양불 판정 엔진 구현",
                    "무인 키오스크(React) 운영, PC·네트워크 없이 USB만으로 안전 업데이트·실패 시 자동 롤백 설계",
                ] },
                { main: "선로전환기 전환력 측정기 개발", sub: [
                    "센서 원시 신호를 실제 전환력 값(kgf)으로 변환, 초당 100회 수집 데이터 저장 (Raspberry Pi)",
